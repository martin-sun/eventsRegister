<script lang="ts">
	...
	const categories = $derived(
		data.categories.map((cat) => ({
			...
			registeredTeams: cat.registered_teams,
			teams: data.teams
		...
		categories
			...
			.map((cat) => ({
				...
				teams: cat.teams.filter((team) => {
					const matchGender =
						activeGenderType === 'all' || team.gender_type === activeGenderType;
					const matchSearch =
						!searchQuery ||
						team.player1_display_name.toLowerCase().includes(searchQuery.toLowerCase()) ||
						team.player2_display_name.toLowerCase().includes(searchQuery.toLowerCase());
					return matchGender && matchSearch;
				})
			}))
	);

	const totalTeams = $derived(data.teams.length);
	...
	<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
		...
		<div class="mx-auto mt-8 flex max-w-lg items-center justify-center gap-8 sm:gap-12">
			{#each categories as cat}
				{@const count = cat.registeredTeams}
				<div class="text-center">
						...
						<table class="w-full">
							...
							<tbody>
								{#each cat.teams as team, i}
									{@const gtl = genderTypeLabels[team.gender_type]}
									<tr class="border-b border-slate-50 transition-colors last:border-0 hover:bg-slate-50/50">
										...
										<!-- Player 1 -->
										<td class="px-4 py-3.5">
											<div class="font-body text-sm font-semibold text-slate-800">{team.player1_display_name}</div>
										</td>
										<!-- Player 2 -->
										<td class="px-4 py-3.5">
											<div class="font-body text-sm font-semibold text-slate-800">{team.player2_display_name}</div>
										</td>
										<!-- Gender type -->
										...
										<!-- Combined age -->
										<td class="px-4 py-3.5 text-center">
											<span class="font-heading text-sm font-bold text-slate-700">{team.combined_age}</span>
						...
						{#each cat.teams as team, i}
							...
							<div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
								<div class="mb-3 flex items-center justify-between">
									...
									<div class="flex items-center gap-2">
										<span class="font-heading text-sm font-bold text-slate-700">{team.combined_age}<span class="font-chinese text-xs font-normal text-slate-400">{m.reg_age_suffix()}</span></span>
										{#if team.status === 'confirmed'}
